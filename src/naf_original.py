...
class NeuralAttentionForest(AttentionForest):
    ...
    def _optimize_weights_end_to_end(self, X, y_orig) -> 'NeuralAttentionForest':
        ...
        background_X = torch.tensor(self.training_xs, dtype=torch.float32)
        
        if scipy.sparse.issparse(self.training_y):
            background_y = self.training_y.toarray()
        else:
            background_y = self.training_y
        background_y = torch.tensor(background_y, dtype=torch.float32)
        
        if len(background_y.shape) == 1:
            background_y = background_y.unsqueeze(1)
            y_orig = y_orig[:, np.newaxis]
        y_true = torch.tensor(y_orig, dtype=torch.long)
    ...
    def predict(self, X, need_attention_weights=False) -> np.ndarray:
        ...
        background_y = torch.tensor(background_y_np, dtype=torch.float32)
